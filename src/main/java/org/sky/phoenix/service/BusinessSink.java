...
public class BusinessSink {
	...
	public static SinkFunction<RawLineage> rawLineageSink(JdbcConfig config) {
		return createJdbcSink(
			config,
			"INSERT IGNORE INTO hive_lineage.raw_lineage(hql_key, raw_lineage, exec_start_time, exec_spend_ms, create_time)\n" +
				"VALUES (?, ?, ?, ?, ?);",
	...
	public static SinkFunction<List<FieldLineage>> fieldLineageSink(JdbcConfig config) {
		return createJdbcSink(
			config,
			"INSERT IGNORE INTO hive_lineage.field_lineage(target_hql_key, target_db, target_table, target_field,\n" +
			...
			(preparedStatement, fieldLineages) -> {
				for (FieldLineage fieldLineage : fieldLineages) {
					preparedStatement.setString(1, fieldLineage.getTargetHqlKey());
					preparedStatement.setString(2, fieldLineage.getTargetDb());
					preparedStatement.setString(3, fieldLineage.getTargetTable());
					preparedStatement.setString(4, fieldLineage.getTargetField());
					preparedStatement.setString(5, fieldLineage.getDownstreamDb());
					preparedStatement.setString(6, fieldLineage.getDownstreamTable());
					preparedStatement.setString(7, fieldLineage.getDownstreamFiled());
					preparedStatement.setTimestamp(8, Timestamp.valueOf(fieldLineage.getCreateTime()));
